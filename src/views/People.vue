      ...
      <div id="container">
        <ion-list>
          <div v-for="person in people" :key="person">
            <ion-item @click="showModal(person.name, person, 'people')">
...
function setSwAPIDataInLocal() {
  axios
    ...
    .then((result) => {
      localStorage.setItem("people", JSON.stringify(result.data.results));
    })
...
function getSwAPIData() {
  let people = [];
  if (JSON.parse(localStorage.getItem("people") as string) !== null) {
    people = JSON.parse(localStorage.getItem("people") as string);
  }
  return people;
}
...
async function presentLoading() {
  const loading = await loadingController.create({
    ...
    message: "Please wait, people are in the dark side...",
    duration: 2000,
  });
...
export default defineComponent({
  ...
  ionViewDidEnter() {
    ...
    //Recovers the information and print it on HTML
    setSwAPIDataInLocal();
    presentLoading();
    setTimeout(() => {
      this.people = getSwAPIData();
    }, 2000);
